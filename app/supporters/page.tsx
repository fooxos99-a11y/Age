                ...
                return (
                  <div className="min-h-screen text-foreground relative overflow-hidden" style={{ backgroundImage: "url('/textures/bg-texture.png')", backgroundSize: 'cover', backgroundRepeat: 'repeat' }}>
                    <div className="relative z-10 max-w-6xl mx-auto py-6 px-2 sm:py-8 sm:px-6">
                      <Header />
                      <div className="flex flex-col items-center mb-8 sm:mb-12 mt-8 sm:mt-[72px]">
                        <h1 className="text-2xl sm:text-4xl font-bold mb-2 text-center" style={{ color: '#f9b83f' }}>
                          الداعمين
                        </h1>
                      </div>
                      <div className="bg-card rounded-xl overflow-hidden border border-border w-full max-w-xs sm:max-w-2xl mx-auto mb-8 sm:mb-12">
                        {/* Table Header */}
                        <div className="grid grid-cols-2 gap-2 sm:gap-4 px-2 sm:px-6 py-2 sm:py-4 bg-card border-b border-border text-xs sm:text-sm font-semibold">
                          <div style={{ color: '#f9b83f' }}>
                            اسم الداعم
                          </div>
                          <div className="text-center" style={{ color: '#f9b83f' }}>
                            المبلغ
                          </div>
                        </div>
                        {/* Table Body */}
                        {supporters.length === 0 ? null : (
                          supporters.map((supporter, idx) => (
                            <div key={idx} className="grid grid-cols-2 gap-2 sm:gap-4 px-2 sm:px-6 py-2 sm:py-3 border-b border-border last:border-b-0 text-base sm:text-lg">
                              <div style={{ color: '#f9b83f' }}>{supporter.name}</div>
                              <div className="text-center" style={{ color: '#f9b83f' }}>{supporter.amount}</div>
                            </div>
                          ))
                        )}
                      </div>
